...
findClustMin <- function(sigma, cluster, clust_pure) {
  ...
  where_absmin <- which(sub_sig_abs == min(colmin), arr.ind=TRUE)
  if (length(where_absmin) > 2) {
    where_absmin <- where_absmin[1, ]
  }
  ## get correlation from not absolute value matrix
  min_corr <- sub_sigma[where_absmin[1], where_absmin[2]]
  return (min_corr)
}
