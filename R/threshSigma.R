#' Threshold \eqn{\hat{\Sigma}} according to false discovery rate threshold specification \code{\thresh}.
...
threshSigma <- function(x, sigma, thresh) {
  ...
  res_adjust <- as.data.frame(matrix(p.adjust(res, method = 'BH'), nrow = nrow(res)))
