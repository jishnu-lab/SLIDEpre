...
```{r init-sigma-hmap, warning = F, message = F, cache = T}
...
#### Inspection of Results
First, we will check that the variables discarded by `plainER()` were actually rescued.

```{r check-rescue, warning = F, message = F, cache = T}
...
intersect_incl_disc <- intersect(incl_genes, disc_genes)
```

Discarded genes: `r indName(disc_genes, colnames(x_data), F)`

Rescued genes: `r indName(intersect_incl_disc, colnames(x_data), F)`
...
```{r plain-sigma, warning = F, message = F, cache = T}
makeHeatmap(mat = as.matrix(initial_run_prior$plainER_results$thresh_sigma), ## sigma hat matrix
            ...
            names = F) ## boolean for displaying column/row names
```

Then $\Delta$:

```{r delta-hmap, warning = F, message = F, cache = T}
...
We can see the variables whose correlations were increased according to the prior information. Finally, $\hat{\Sigma}_{prior}$:

```{r prior-sigma, warning = F, message = F, cache = T}
...
$\hat{\Sigma}_{prior}$ is a weighted average of $\hat{\Sigma}$ and $\Delta$.`priorER()` uses a function, `makeDelta()`, which implements the correlation matrix shrinkage estimation procedure mentioned above. This algorithm uses maximum likelihood to find the optimal weight, $\alpha$. In this case, the algorithm identified $\alpha$ to be `r initial_run_prior$priorER_sigma$optim_alpha`, and we can visualize the process with `marg_plot` from the `priorER()` output:

```{r marg-plot, warning = F, message = F}
initial_run_prior$priorER_sigma$marg_plot
```
...
correction: TRUE          # do multiple testing correction?
verbose: TRUE             # include print statements?
thresh_fdr: 0.2           # false discovery rate thresholding p-value cutoff
change_all: FALSE         # change entire row/column for important variables?
```

```{r pipeline1, warning = F, message = F, eval = F}
pipe1_run <- pipelineER1(yaml_path = "/Users/aerosengart/Documents/Das Lab/EssReg/data/pipeline1.yaml", steps = "all")
```
